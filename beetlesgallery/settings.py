...
# 1. Capture the production version if available
APP_VERSION = os.environ.get('APP_VERSION')

# 2. Fallback for development: Get short git commit hash
if not APP_VERSION:
    try:
        # Runs 'git rev-parse --short HEAD' to get the current hash (e.g., a1b2c3d)
        APP_VERSION = subprocess.check_output(['git', 'rev-parse', '--short', 'HEAD']).decode('utf-8').strip()
    except Exception:
        APP_VERSION = 'dev-build'

# 3. Ensure DEBUG is accessible to determine environment in templates
