...
class Command(BaseCommand):
    help = "Publish/replace the valid_species.csv reference and update caches."

    def add_arguments(self, parser):
        parser.add_argument(
            "--src",
            required=True,
            help="Path to the valid_species.csv to publish",
        )
        parser.add_argument(
            "--label",
    ...
    def handle(self, *args, **opts):
        ...
        try:
            rows, version = species_ref.publish_from_file(str(src), label=label)
